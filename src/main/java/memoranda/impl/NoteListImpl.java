 ...
 */
package main.java.memoranda.impl;
...
//    public static final String NS_JNNL = "http://www.openmechanics.org/2003/jnotes-noteslist";
    ...
    public Collection getNotesForPeriod(CalendarDate startDate, CalendarDate endDate) {
        ...
        for (int yi = 0; yi < yrs.size(); yi++) {
            Year y = new Year(yrs.get(yi));
            if ((y.getYearValue() >= startDate.getYear()) && (y.getYearValue() <= endDate.getYear())) {
                Vector months = y.getMonths();
                for (int mi = 0; mi < months.size(); mi++) {
                    Month m = (Month) months.get(mi);
                    if (!((y.getYearValue() == startDate.getYear()) && (m.getMonthValue() < startDate.getMonth()))
                        || !((y.getYearValue() == endDate.getYear()) && (m.getMonthValue() > endDate.getMonth()))) {
                        Vector days = m.getDays();
                        for (int di = 0; di < days.size(); di++) {
                            Day d = (Day) days.get(di);
                            if (!((m.getMonthValue() == startDate.getMonth()) && (d.getDayValue() < startDate.getDay()))
							|| !((m.getMonthValue() == endDate.getMonth()) && (d.getDayValue() > endDate.getDay()))) {
								Vector ns = d.getNotes();
								for(int ni = 0; ni < ns.size(); ni++) {
									NoteElement n = (NoteElement) ns.get(ni);
									v.add(new NoteImpl(n.getElement(), _project));
...
*/
    ...
    private class Year {
        ...
        // Duplicate code. public int getValue() is declared several different 
        // times in this class, each with similar return values.
